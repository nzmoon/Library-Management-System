...
class Books{
    ...
    public function showAllBooks(){
        ...
        $sql = "SELECT book.id, book.book_name,book.sbn, book.quantity,book.publishing_year,book.description,book.pdf_link, books_category.cat_name,publisher.publisher_name,writer.writer_name FROM book,books_category,publisher,writer WHERE book.`book_cat`= books_category.cat_id AND book.`publisher_id`= publisher.id AND book.`writer_id`= writer.id ";
        $conn = $DBH->prepare($sql);
        $conn->execute();
        $allBooks= $conn->fetchAll(PDO::FETCH_OBJ);
    ...
    public function countbookquantity($book_id){
        $obj = new DataBase();
        $DBH=$obj->DBconnection();

        $sql = "SELECT book.quantity FROM `book` WHERE `id`='$book_id'";
        $conn= $DBH->prepare($sql);
        $conn->execute();
        $quantity=$conn->fetch(PDO::FETCH_OBJ);
    ...
    public function updateCategory($cat_id, $cat_name){
        ...
        $sql = "UPDATE `books_category` SET `cat_name`='$cat_name' WHERE `cat_id`='$cat_id'";
        $conn = $DBH->prepare($sql);
        $conn->execute();
    }
